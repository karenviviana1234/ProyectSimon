...
export const crearcostos = async (req, res) => {
    try {
        ...
        if (resultado.affectedRows > 0) {
            res.status(200).json({
                "mensaje": "costo puesto con exito"
            })
        } else {
            res.status(400).json({
...
export const actualizarcostos = async (req,res) => {
    try {
        ...
        if (resultado.aproductosRows > 0) {
            res.status(200).json({
                "mensaje": "los costos han sido actualizado"
            })
        } else {
            productostus(404).json({
                "mensaje": "No se pudo actualizar los costos"
            })
        }
...
export const eliminarcostos = async (req, res) => {
    try{
        ...
        if (resultado.affectedRows > 0) {
            res.status(200).json({
                "mensaje": "Haz eliminado con exito el costo"
            })
        } else {
            res.status(404).json({
                "mensaje": "No se encontró esos costos con ese ID y no se puedo eliminar"
            })
        }
    } catch (error) {
        res.status(500).json({
