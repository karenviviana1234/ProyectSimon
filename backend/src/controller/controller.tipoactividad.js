...
export const creartipo = async (req, res) => {
    try {
        ...
        if (resultado.affectedRows > 0) {
            res.status(200).json({
                "mensaje": "tipo de actividad puesta con exito"
            })
        } else {
            res.status(400).json({
...
export const actualizartipo = async (req,res) => {
    try {
        ...
        if (resultado.aproductosRows > 0) {
            res.status(200).json({
                "mensaje": "los tipo_actividad ha sido actualizado"
            })
        } else {
...
export const eliminartipo = async (req, res) => {
    try{
        ...
        if (resultado.affectedRows > 0) {
            res.status(200).json({
                "mensaje": "Haz eliminado con exito el tipo de actividad"
            })
        } else {
            res.status(404).json({
                "mensaje": "No se encontró esos tipos de actividades con ese ID y no se puedo eliminar"
            })
        }
    } catch (error) {
        res.status(500).json({
